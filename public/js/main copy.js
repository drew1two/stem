...
//jquery datepicker
$(".pick-date").datepicker({dateFormat: "dd-mm-yy", minDate: 0});
//jquery selectbox
$(".select-box").selectbox();
// search box
$("#gquery").keyup(function(event){
    if(event.keyCode == 13){
        $("#gsearch_go").click();
    }
});
$('#gsearch_go').click(function(){
	var query = document.getElementById("gquery").value;
	window.open("http://www.google.com/cse?cx=012033335055527637981:3e3i3wqobss&ie=UTF-8&q="+query);
});
	//mobile menu
	...
	$(".slider").carouFredSel({
		responsive: true,
		prev: {
			button  : ".slider-nav-prev, .slider-nav-prev-480",
			key     : "left",
		...
		next: {
			button  : ".slider-nav-next, .slider-nav-next-480",
			key     : "right",
			onAfter: onAfterSlide,
			onBefore: onBeforeSlide,
			easing: "easeInOutQuint",
...
var hideBlock = $(".gallery-hide");
		//open gallery details
		if(location.hash.substr(1,21)=="gallery-details-close" || typeof(hashOptions.filter)!="undefined")
		{
var hideBlockVisible=parseInt(hideBlock.css("height"))==0 ? false : true;
if(!hideBlockVisible)
{
	hideBlock.css("display", "block").animate({height:hideBlock.height()}, 500, 'easeOutQuint', function(){
		$(this).css("height", "100%");
	});
	$("html, body").animate({scrollTop: $("html, body").top}, 400);
}
else
{
	$("html, body").animate({scrollTop: $("html, body").top}, 400);
		...
		{
hideBlock.animate({height:'0'},{duration:200,easing:'easeOutQuint', complete:function(){
...
function form_chk()
{
	error = "Please enter the following information:\n";
	error_parts="";
	gender_check = 0;
	obj=document.forms["paypalForm"];

	var inv_no=obj.invNo.value;
	var email_chk=IsEmailValid(obj.os5.value);

	if(obj.invNo.value=="")
		error_parts = error_parts + "- Invoice No\n";
	else if (isNaN(inv_no)==true)
		error_parts = error_parts + "- Your Invoice No should contain only digits\n";
	else if (inv_no.length!=8)
		error_parts = error_parts + "- Your Invoice No should contain 8 digits\n";
	else
		obj.item_number.value=obj.invType.value+obj.invNo.value;
	
	if(obj.amount.value=="")
		error_parts = error_parts + "- Invoice Amount\n";
	
	if(obj.os1.value=="")
		error_parts = error_parts + "- SCP No\n";
	
	if(obj.os2.value=="")
		error_parts = error_parts + "- Customer Name\n";
	
	if(obj.os3.value=="")
		error_parts = error_parts + "- Paypal Name/Email\n";
		
	if(obj.os4.value=="")
		error_parts = error_parts + "- Contact No\n";
	
	if(obj.os5.value=="")
		error_parts = error_parts + "- Email Address\n";
	else if(email_chk==false)
		error_parts = error_parts + "- Correct Email Format\n";

	if (error_parts != ""){
		alert(error + error_parts);
		return false;
	}
	else
	{
